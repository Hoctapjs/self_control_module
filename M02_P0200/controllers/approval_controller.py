...
class ApprovalController(CustomerPortal):
    ...
    def _get_user_activity_display_map(self, activities):
        ...
        ops_offboarding_category = request.env.ref(
            'M02_P0213.psm_0213_approval_category_resignation',
            raise_if_not_found=False,
        )
        rst_offboarding_category = request.env.ref(
        ...
        for activity in activities:
            ...
            is_ops_offboarding_activity = bool(
                ops_offboarding_category
                and hasattr(activity, '_is_ops_offboarding_activity')
                and activity._is_ops_offboarding_activity(activity, ops_offboarding_category)
            )
            is_rst_offboarding_activity = bool(
                rst_offboarding_category
                and hasattr(activity, '_is_rst_offboarding_activity')
                and activity._is_rst_offboarding_activity(activity, rst_offboarding_category)
            )
            hide_model_tag = is_ops_offboarding_activity or is_rst_offboarding_activity
    ...
    def portal_finalize_resignation(self, request_id, action_code):
        def _extract_notification(result):
        ...
        try:
            ...
            if action_code == 'ops_send_social_insurance':
                result = approval.action_send_social_insurance()
            elif action_code == 'ops_send_adecco_notification':
                result = approval.action_send_adecco_notification()
            elif action_code == 'ops_done':
                result = approval.action_done()
        ...
        notification = _extract_notification(result)
        if notification and notification['type'] in ('danger', 'warning'):
            return {'error': notification['message'] or 'KhÃ´ng thá»ƒ hoÃ n táº¥t thao tÃ¡c.'}

        return {
            'success': True,
            'message': (
                notification['message']
                if notification and notification['type'] == 'success' and notification['message']
                else allowed_actions[action_code]['success_message']
            ),
